-- Script SQL corrigido para limpeza total com nomes de tabelas corretos
-- RESOLVE definitivamente as 11 inconsistências críticas
...
DELETE FROM configuracoes WHERE TRUE;

-- 1.7. Verificação de limpeza
DO $$
DECLARE
...
BEGIN
    SELECT 
        ...
        (SELECT COUNT(*) FROM fichas_epi) +
        (SELECT COUNT(*) FROM colaboradores) +
        (SELECT COUNT(*) FROM tipos_epi)
    INTO total_registros;
    
    IF total_registros > 0 THEN
        RAISE EXCEPTION 'ERRO: Limpeza incompleta. Ainda existem % registros', total_registros;
    END IF;
...
FROM (
    ...
    LEFT JOIN movimentacoes_estoque me ON me.estoque_item_id = ei.id
